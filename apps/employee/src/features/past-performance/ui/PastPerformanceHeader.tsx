    ...
    <section className="relative left-1/2 -mx-[50vw] w-screen bg-[linear-gradient(90deg,#6179e7_0%,#7b4ca5_100%)] text-white">
      <div className="container mx-auto flex flex-col gap-5 px-6 py-7 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight sm:text-[2.2rem]">過去の実績</h1>
            <p className="mt-1 text-sm font-semibold text-white/85 sm:text-[1.05rem]">コレクレ管理システム</p>
          </div>
        </div>

        <div className="flex items-center gap-4 self-end sm:self-auto">
          <div className="text-right">
            <div className="text-xs font-semibold tracking-[0.2em] text-white/70">{departmentName || "従業員"}</div>
            <div className="mt-1 text-lg font-bold sm:text-[1.7rem]">{employeeName}</div>
          </div>
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/18 text-lg shadow-lg shadow-violet-900/15">
            <FontAwesomeIcon icon={faUser} style={{ color: "#fff", width: "1.5rem", height: "1.5rem" }} />
          </div>
        </div>
      </div>
    </section>
