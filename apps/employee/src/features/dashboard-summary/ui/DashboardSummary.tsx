...
import { faCalendarCheck, faClock, faCoins, faTrophy } from "@fortawesome/free-solid-svg-icons";
...
export default function DashboardSummary({ companyId, userId, targetYearMonth }: Props) {
  ...
  if (loading || (!summary && !error)) {
    return (
      <div className="-mx-6 flex gap-4 overflow-x-auto overflow-y-visible px-6 py-4 md:grid md:grid-cols-2 xl:grid-cols-4">
  ...
  return (
    <div className="-mx-6 flex gap-4 overflow-x-auto overflow-y-visible px-6 py-4 md:grid md:grid-cols-2 xl:grid-cols-4">
      ...
      <ScoreTile
        ...
        iconBgColor="#ede9fe"
        label="翌月反映予定"
        value={summary.pendingPointBalance}
        unit="ポイント"
        color="#7C3AED"
      />
      <ScoreTile
