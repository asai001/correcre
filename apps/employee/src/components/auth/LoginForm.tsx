...
function buildForgotPasswordHref(redirectTo: string) {
  if (redirectTo === EMPLOYEE_DEFAULT_REDIRECT_PATH) {
    return EMPLOYEE_FORGOT_PASSWORD_PATH;
  }

  const params = new URLSearchParams({ from: redirectTo });
  return `${EMPLOYEE_FORGOT_PASSWORD_PATH}?${params.toString()}`;
}
...
export default function LoginForm({
  ...
  useEffect(() => {
    ...
    document.cookie = `${EMPLOYEE_LOGIN_NOTICE_COOKIE_NAME}=; Max-Age=0; Path=/; SameSite=Lax`;
  }, [noticeMessage]);
