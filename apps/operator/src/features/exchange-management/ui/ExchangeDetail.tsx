...
type Props = {
  initial: OperatorExchangeDetail;
  operatorName: string;
};
...
export default function ExchangeDetail({ initial, operatorName }: Props) {
  ...
  const handleTransition = (nextStatus: ExchangeHistoryStatus) => {
    ...
    startTransition(async () => {
      try {
        const updated = await transitionExchange(detail.merchantId, detail.exchangeId, {
          nextStatus,
    ...
    <div className="space-y-6 pb-10">
      ...
      <section className="rounded-[28px] bg-white p-6 shadow-lg shadow-slate-200/70">
        ...
        <dl className="mt-6 grid gap-4 text-sm md:grid-cols-2">
          <div>
            <dt className="text-xs font-semibold text-slate-500">提携企業</dt>
            <dd className="mt-1 text-slate-900">
              {detail.merchantName ?? detail.merchantId}
              <span className="ml-2 text-xs text-slate-500">({detail.merchantId})</span>
            </dd>
          </div>
          <div>
