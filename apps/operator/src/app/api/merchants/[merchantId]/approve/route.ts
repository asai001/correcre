...
import { approveMerchantApplicationForOperator } from "@operator/features/merchant-management/api/server";
...
export async function POST(_req: Request, context: RouteContext) {
  ...
  try {
    const merchant = await approveMerchantApplicationForOperator({ merchantId }, access.user);
    return NextResponse.json(merchant);
  } catch (err) {
    console.error("POST /api/merchants/[merchantId]/approve error", err);
    ...
    if (err instanceof Error) {
      const status =
        err.message === "Merchant not found"
          ? 404
