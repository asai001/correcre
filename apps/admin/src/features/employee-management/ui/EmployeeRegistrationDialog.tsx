...
export default function EmployeeRegistrationDialog({
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pb: 1 }}>
        <div className="text-2xl font-bold text-slate-900">新規従業員登録</div>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
          従業員一覧に追加するための基本情報を入力します。部署は複数選択でき、登録後は保有ポイント 0pt で追加されます。
        </Typography>
      </DialogTitle>
              ...
              >
                {departmentOptions.map((departmentOption) => (
                  <MenuItem key={departmentOption.name} value={departmentOption.name}>
                    ...
                    <ListItemText
                      primary={departmentOption.name}
                      secondary={`${departmentOption.employeeCount}名所属`}
                    />
                  </MenuItem>
                ))}
              </Select>
