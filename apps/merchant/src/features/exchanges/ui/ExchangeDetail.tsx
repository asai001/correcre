...
type ExchangeHistoryEvent = ExchangeDetailType["history"][number];
      ...
      {MAIN_FLOW_STATUSES.map((flowStatus, index) => {
        ...
        return (
          <li key={flowStatus} className="flex items-center gap-1.5">
            <span className={`rounded-full px-3 py-1 text-xs ${chipClassName(state)}`}>{badge.label}</span>
...
type ApplicantAddress = NonNullable<ExchangeDetailType["applicantAddress"]>;
    ...
    <div className="space-y-6 pb-10">
      ...
      <section className="rounded-[28px] bg-white p-6 shadow-lg shadow-slate-200/70">
        ...
        <dl className="mt-6 grid gap-4 text-sm md:grid-cols-2">
          ...
          <div>
            <dt className="text-xs font-semibold text-slate-500">企業名</dt>
